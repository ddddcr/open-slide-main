    ...
    <aside className="paper relative flex h-full w-[16.5rem] shrink-0 flex-col border-r border-hairline bg-sidebar text-sidebar-foreground">
      ...
      <div className="px-2">
        <button
          ...
          onClick={onSelectAll}
          className={
            'group relative flex w-full items-center gap-2.5 rounded-[5px] px-2 py-[5px] text-[12.5px] transition-colors ' +
            (selectedId === ALL_ID
        ...
        >
          ...
          <span className="min-w-0 flex-1 truncate text-left">{t.home.allSlides}</span>
          <span className="folio ml-auto shrink-0">{countAll.toString().padStart(2, '0')}</span>
        </button>
        <FolderItem
          row={{ kind: 'draft' }}
          count={countFor(null)}
